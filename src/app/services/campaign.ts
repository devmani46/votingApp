...
export class CampaignService {
        ...
        list.map((c) => {
          if (c.id === data.campaignId) {
            return {
              ...c,
              candidates: c.candidates.map((cd) =>
                cd.id === data.candidateId ? { ...cd, ...data.changes } : cd
              ),
            };
          }
          return c;
  ...
  addCampaign(
    campaign: Omit<Campaign, 'id' | 'candidates'>
  ): Observable<Campaign> {
    return this.http
      .post<Campaign>(`${this.apiUrl}/campaigns`, campaign, {
  ...
  updateCampaign(id: string, updated: Partial<Campaign>): Observable<Campaign> {
    return this.http
      .put<Campaign>(`${this.apiUrl}/campaigns/${id}`, updated, {
        withCredentials: true,
      })
      .pipe(
        tap((updatedCampaign) => {
  ...
  deleteCampaign(id: string): Observable<void> {
    return this.http
      .delete<void>(`${this.apiUrl}/campaigns/${id}`, { withCredentials: true })
      .pipe(
        tap(() => {
  ...
  addCandidate(
    ...
    return this.http
      .post<Candidate>(
        `${this.apiUrl}/campaigns/${campaignId}/candidates`,
        candidate,
        { withCredentials: true }
      )
      .pipe(
  ...
  deleteCandidate(campaignId: string, candidateId: string): Observable<void> {
    return this.http.delete<void>(
      `${this.apiUrl}/campaigns/${campaignId}/candidates/${candidateId}`,
      { withCredentials: true }
    );
  }
            ...
            list.map((c) => {
              if (c.id === campaignId) {
                ...
                return {
                  ...c,
                  candidates: c.candidates.map((cd) =>
                    cd.id === candidateId ? updatedCandidate : cd
                  ),
                };
              }
              return c;
  ...
  castVote(campaignId: string, candidateId: string): Observable<any> {
    return this.http
      .post(
        `${this.apiUrl}/campaigns/${campaignId}/vote`,
