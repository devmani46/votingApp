...
export class CreateCampaign {
  ...
  headerTitle = computed(() =>
    this.editMode() ? 'Edit Campaign' : 'Create Campaign'
  );
  submitButtonLabel = computed(() =>
    this.editMode() ? 'Update Campaign' : 'Create Campaign'
  );
      ...
      if (candidate.id) {
        try {
          await this.campaignService
            .deleteCandidate(this.campaignId()!, candidate.id)
            .toPromise();
  ...
  async submitForm() {
    ...
    try {
      if (this.editMode() && this.campaignId() !== null) {
        ...
        const existingCampaign = this.campaignService.getCampaignById(
          this.campaignId()!
        );
        if (existingCampaign) {
          const existingCandidates = existingCampaign.candidates || [];
          const formCandidates = (formValue.candidates || []) as {
            id?: string;
            name: string;
            bio?: string;
            photo?: string;
          ...
          for (const existingCandidate of existingCandidates) {
            if (!formCandidates.find((c) => c.id === existingCandidate.id)) {
              await this.campaignService
                .deleteCandidate(this.campaignId()!, existingCandidate.id)
                .toPromise();
            }
          }
          ...
          for (const c of formCandidates) {
            if (c.id) {
              await this.campaignService
                .updateCandidate(this.campaignId()!, c.id, {
                  name: c.name,
                  bio: c.bio || '',
                  photo_url: c.photo || '',
                })
        ...
        const createdCampaign = await this.campaignService
          .addCampaign(campaignData)
          .toPromise();
        if (!createdCampaign || !createdCampaign.id)
          throw new Error('Missing ID');
        for (const c of (formValue.candidates || []) as {
          id?: string;
          name: string;
          bio?: string;
          photo?: string;
          ...
          await this.campaignService
            .addCandidate(createdCampaign.id, {
              name: c.name,
              bio: c.bio || '',
              photo_url: c.photo || '',
            })
